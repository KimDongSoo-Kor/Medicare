...
export const Receipt: React.FC<Props> = ({ data }) => {
    ...
    // Chunk the rows into arrays of ROWS_PER_PAGE
    const chunks = [];
    for (let i = 0; i < dailyRows.length; i += ROWS_PER_PAGE) {
        chunks.push(dailyRows.slice(i, i + ROWS_PER_PAGE));
    }
    ...
    return (
        <div id="receipt-certificate" className="flex flex-col" style={{ gap: '2rem' }}>
            {chunks.map((chunkRows, pageIndex) => {
                ...
                const chunkTotalAmount = chunkRows.reduce((sum, row) => sum + row.amount, 0);
                        ...
                        <table className="w-full border-collapse mb-0">
                            <tbody>
                                <tr>
                                    ...
                                    <td className="border border-gray-400 px-2 py-2 text-center font-bold">₩{chunkTotalAmount.toLocaleString()}</td>
                        ...
                        <table className="w-full border-collapse">
                            ...
                            <tbody>
                                ...
                                <tr className="font-bold">
                                    <td className="border border-gray-400 px-2 py-1 text-center text-sm">
                                        {isLastPage ? '총 계' : '소 계'}
                                    </td>
                                    <td className="border border-gray-400 px-2 py-1 text-center text-xs text-gray-500">
                                        {chunks.length > 1 ? `(${pageIndex + 1}/${chunks.length}페이지)` : ''}
                                    </td>
                                    <td className="border border-gray-400 px-2 py-1 text-center text-sm">
                                        {isLastPage ? totalQty : chunkQty}
                                    </td>
                                    <td className="border border-gray-400 px-2 py-1 text-center text-sm"></td>
                                    <td className="border border-gray-400 px-2 py-1 text-right text-sm pr-3">
                                        {isLastPage ? (data.totalAmount?.toLocaleString() || '') : chunkTotalAmount.toLocaleString()}
                                    </td>
                                </tr>
                            </tbody>
                        </table>
